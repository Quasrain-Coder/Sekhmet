...
async def auto_bot_actions(table_id: str) -> list[dict[str, Any]]:
    ...
    Call this after a human action or after dealing a new hand.  Every bot
    action waits bot_action_delay_seconds before it is executed (like a
    real player thinking) and is broadcast immediately, so clients see
    each move as it happens instead of a burst at the end of the cascade.

    Returns an empty list (kept for callers written against the old
    batching contract — broadcasts now happen inline).
    """
    ...
    for _ in range(max_iterations):
        ...
        # Read phase under the lock: a grace-expiry (_expire_seat) or a
        # second auto_bot_actions loop must never interleave with our
        # read → step → write cycle, or both sides step from stale
        # snapshots (duplicated streets, the pot resolved twice, or a
        # stale write resurrecting pre-surgery state).
        async with session.lock:
            gs = session.game_state
            ...
            if cur_idx is None:
                ...
                # broadcast it live from inside the loop.  Sleeping before
                # each step (including the first) keeps the cadence
                # uniform: FLOP → delay → TURN → delay → RIVER → delay →
                # result.  The sleep stays inside the lock on purpose —
                # the runout is a single critical section.
        ...
        # Bot's turn — pause OUTSIDE the lock (a human acting during the
        # pause must not be blocked), then re-take it and only act if the
        # game has not moved on: every action replaces the immutable
        # GameState snapshot, so an identity check is a full revalidation.
        await asyncio.sleep(app_config.game.bot_action_delay_seconds)

        async with session.lock:
            if session.game_state is not gs:
                continue  # someone acted during the pause — re-evaluate

            # Decide and execute.  A bot that crashes or produces an
            # illegal action must never freeze the game: fall back to
            # check (free) or fold (facing a bet) and carry on.
...
async def after_action(table_id: str) -> None:
    ...
    try:
        # auto_bot_actions broadcasts each bot move inline now (so players
        # see bots act one by one with the configured delay between them).
        await auto_bot_actions(table_id)
    except Exception:
